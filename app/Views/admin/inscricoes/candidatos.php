...
<div class="d-flex flex-wrap justify-content-between align-items-end mb-3 gap-2">
  ...
  <div class="d-flex gap-2">
    ...
    <a class="btn btn-cns-contorno" href="<?= site_url('admin/candidatos/exportar') . ($qs ? '?' . $qs : '') ?>">
...
<form method="get" action="<?= site_url('admin/candidatos') ?>" class="cartao p-3 mb-3">
  <div class="row g-2 align-items-end">
    <div class="col-md-4">
      ...
      <input class="form-control form-control-sm" id="q" name="q" value="<?= esc($filtros['q'], 'attr') ?>"
      ...
      <div class="col-md-2">
        ...
        <select class="form-select form-select-sm" id="<?= $nome ?>" name="<?= $nome ?>">
          ...
          <?php foreach ($lista as $id => $texto): ?>
            <option value="<?= (int) $id ?>" <?= (string) $filtros[$nome] === (string) $id ? 'selected' : '' ?>>
              <?= esc($texto) ?>
            </option>
          <?php endforeach ?>
        </select>
